...
const DiscountManagement = () => {
    ...
    const addNewPromotion = (newPromotion) => {
        setFilteredRows((prevRows) => [...prevRows, newPromotion]);
    ...
    const updatePromotion = (updatedName, updatedDateStart, updatedDateEnd, updatedPresentDiscount) => {
    ...
    const handleDeletePromotion = () => {
        ...
        const isConfirmed = window.confirm("Bạn có chắc muốn xóa khuyến mãi này không?");
        if (isConfirmed) {
            setFilteredRows((prevRows) =>
        ...
        <div>
                ...
                <div className={styles.genaralContainer}>
                    ...
                    <div className={styles.toolsManagerContainer}>
                        <button className={styles.addButotn}>
                            <InlineIcon className={styles.icon} icon="ic:round-add" onClick={handleAddPromotionButtonCLick}></InlineIcon>
                        </button>
                        <button className={styles.deleteButotn}>
                            <InlineIcon className={styles.icon} icon="pepicons-pop:line-x" onClick={handleDeletePromotion}></InlineIcon>
                        </button>
                        <button className={styles.updateButotn}>
                            <InlineIcon className={styles.icon} icon="bxs:pencil" onClick={handleOpenUpdatePromotionButtonClick}></InlineIcon>
                        </button>
                    </div>
                </div>

                <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <Paper sx={{ height: "646px", width: "70%", margin: "0 30px", marginTop: "67px", overflowY: "hidden" }}>
                        <DataGrid
                            rows={filteredRows}
                            columns={columns}
            ...
            {isAddPromotionOpen && (
                <AddPromotion
                    ...
                    onAddPromotion={addNewPromotion}></AddPromotion>
            )}
            ...
            {isUpdatePromotionOpen && (
                <UpdatePromotion
                    ...
                    percentDiscount={selectedRowId.percentDiscount}></UpdatePromotion>
            )}
