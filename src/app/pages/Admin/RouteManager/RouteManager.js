...
const RouteManager = () => {
    ...
    const updateTicketCost = (updateDate, updateTimeDuring, updateLocationFrom, upadteLocationTo, updateStationFrom, updateStationTo, updateCostNormal, updateCostVIP) => {
    ...
    const handleSearching = () => {
        const filteredData = rows.filter(row => {
            ...
            return (
                (!locationFrom || (row.locationFrom && row.locationFrom.includes(locationFrom))) &&
                (!locationTo || (row.locationTo && row.locationTo.includes(locationTo))) &&
        ...
        <div>
                ...
                <div className={styles.genaralContainer}>
                    ...
                    <div className={styles.toolsManagerContainer}>
                        <button className={styles.addButotn}>
                            <InlineIcon className={styles.icon} icon="ic:round-add" onClick={handleAddTicketButtonCLick}></InlineIcon>
                        </button>
                        <button className={styles.deleteButotn}>
                            <InlineIcon className={styles.icon} icon="pepicons-pop:line-x" onClick={handleDeleteTicket}></InlineIcon>
                        </button>
                        <button className={styles.updateButotn}>
                            <InlineIcon className={styles.icon} icon="bxs:pencil" onClick={handleOpenUpdateTicketButtonClick}></InlineIcon>
                        </button>
                    </div>
                </div>

                <Paper sx={{ height: "646px", width: "83%", margin: "0 30px", marginTop: "67px", overflowY: "hidden", justifySelf: "center" }}>
                    <DataGrid
                        rows={fillteredRows}
                        columns={columns}
            ...
            {isAddTicketOpen && (
                <AddTicket
                    ...
                    onAddTicket={addNewTicket}></AddTicket>
            )}
