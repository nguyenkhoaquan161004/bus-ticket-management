...
const SearchingTicket = ({
  onPrintBoxStateChange,
  onChangeTicketBoxStateChange,
}) => {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5278/check-ticket/${ticketCode}/${phoneNumber}`
      );
  ...
  const handleCancel = async () => {
    try {
      const response = await axios.post(
        `http://localhost:5278/cancel-ticket-request/${row.ticketId}`
      );
  ...
  return (
    <div>
      ...
      {/* RESEARCH RESULT */}
      {isResultOpen && (
        <div className={styles.researchResultContainer}>
            ...
            <div className={styles.resultList}>
              <div className={styles.resultItemContainer}>
                ...
                <div className={styles.buttonField}>
                  {isEmployee && (
                    <button
                      style={{ background: "#2E6B75" }}
                      onClick={handleOpenPrintBox}
                    >
                      <p className="uiSemibold">In vé</p>
                    </button>
                  )}

                  <button
                    style={{ background: "#D7987D" }}
                    onClick={handleOpenChangeTicketBox}
                  >
                    <p className="uiSemibold">Đổi vé</p>
                  </button>

                  <button
                    style={{ background: "#D24F4F" }}
                  ...
                  <div className={styles.boxBackground}>
                    ...
                    {isChangeTicketBoxOpen && (
                      <div className={styles.printBoxContainer}>
                        ...
                        <div className={styles.printFlexBoxContainer}>
                          <div className={styles.inforSpaceContainer}>
                            <div className={styles.costTicket}></div>
                            <hr />
                            <div className={styles.inforTicket}>
                              <div className={styles.itemInfor}>
                                <h4>Mã vé: </h4>
